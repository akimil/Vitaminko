...
public class AuraluxTest extends ApplicationAdapter {
	...
	private final int SOLDIER_SPEED 	   = 5;					// The speed of soldiers, ie. the number of pixels that soldier passes in 1 second
	private final int ACCELERATION		   = 1;					// Maximal acceleration that soldier can attain
	...
	*/
		...
		public boolean touchUp(int screenX, int screenY, int pointer, int button) { 
			if(selecting){
				for(int i=0; i<soldiers.size(); i++)if(soldiers.get(i).x <= selectedCircle.x + selectedCircle.radius && soldiers.get(i).x >= selectedCircle.x - selectedCircle.radius && Gdx.graphics.getHeight() - soldiers.get(i).y <= selectedCircle.y + selectedCircle.radius && Gdx.graphics.getHeight() - soldiers.get(i).y >= selectedCircle.y - selectedCircle.radius)if(selectedCircle.contains(soldiers.get(i).x, Gdx.graphics.getHeight() - soldiers.get(i).y) && selectedCircle.contains(soldiers.get(i).x, Gdx.graphics.getHeight() - soldiers.get(i).y)){
					...
					if(soldiers.get(i).px>=soldiers.get(i).x)
					{
						soldiers.get(i).ax=ACCELERATION;
					}
					else
					{
						soldiers.get(i).ax=-1*ACCELERATION;
					}
					if(soldiers.get(i).py>=soldiers.get(i).y)
					{
						soldiers.get(i).ay=ACCELERATION;
					}
					else
					{
						soldiers.get(i).ay=-1*ACCELERATION;
					}
	...
	private class Soldier {
		...
		public void update(){
			...
			// Motion
			if(this.vx*this.vx>2*Math.abs(this.px-this.x)*ACCELERATION)
			{
				if(this.px>this.x)
				{
					ax=-1*ACCELERATION;
			...
			if(this.vy*this.vy>2*Math.abs(this.py-this.y)*ACCELERATION)
			{
				if(this.py>this.y)
				{
					ay=-1*ACCELERATION;
			...
			// Collision detector - not optimal at all, but simple--v1.2 optimization, check this only if the soldier is moving
			if(vx*vx+vy*vy>0)
			{
			...
			for(i=0; i<barracks.size(); i++)
			{
				...
				if(barracks.get(i).team != this.team && barracks.get(i).team != 0 && pitagora(this.x - barracks.get(i).x, this.y - barracks.get(i).y) < SUN_ATTACK && barracks.get(i).x==this.px && barracks.get(i).y==this.py)
				{
					...
					if(barracks.get(i).health <= 0)barracks.get(i).changeTeam((byte)0);			// Change team if lost health
					break;
				}
	...
	private class Barrack {
		...
		public void update(){
			...
			if(step == SUN_STEP){
				step = 0;
				soldiers.add(new Soldier(x + (rnd.nextFloat() * 40) - 20f, y + (rnd.nextFloat() * 40) - 20f,0 , 0, team));
			}
			if(health <= 0){
				health = MAX_HEALTH;
				team = 1;
